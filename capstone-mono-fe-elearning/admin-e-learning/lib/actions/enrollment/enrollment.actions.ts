 ...
 */
export async function getEnrollments(page: number = 1, size: number = 10, search: string = ''): Promise<EnrollmentResponse> {
    let url = `${API_BACKEND_URL}/enrollments?page=${page}&size=${size}`;
    ...
    const response = await get<EnrollmentResponse>(url);
