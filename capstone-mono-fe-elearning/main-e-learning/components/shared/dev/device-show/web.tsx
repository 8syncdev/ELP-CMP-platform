...
// Định nghĩa các hướng crop có thể kết hợp
type CropDirection = 'top' | 'bottom' | 'left' | 'right' | 'center';

interface CropSettings {
    directions: CropDirection[];  // Mảng các hướng crop
    intensity?: number;  // Độ mạnh của crop (0-100)
}
...
export function Safari({
    ...
    height = 753,
    mode = "default",
    crop = { directions: ['center'], intensity: 0 },
    className,
    ...props
}: SafariProps) {
    // Tính toán tỷ lệ responsive
    const aspectRatio = width / height;

    // Tính toán giá trị object-position và object-fit dựa trên crop settings
    const calculateCropStyles = (crop: CropSettings) => {
        const { directions, intensity = 0 } = crop;
        const normalizedIntensity = Math.min(Math.max(intensity, 0), 100);

        // Chỉ dùng translate để di chuyển theo hướng được chọn
        let translateX = 0;
        let translateY = 0;

        if (directions.includes('top')) {
            translateY = -normalizedIntensity;
        }
        if (directions.includes('bottom')) {
            translateY = normalizedIntensity;
        }
        if (directions.includes('left')) {
            translateX = -normalizedIntensity;
        }
    ...
    const cropStyles = calculateCropStyles(crop);
